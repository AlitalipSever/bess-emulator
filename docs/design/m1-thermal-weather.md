...
- **Golden snapshots:** planned for exactly twice (PR2 dataset switch, PR3
  physics change). It became three: PR4 adds the HVAC mode and its
  anti short-cycle timer to the state tree, and any field added to the tree
  moves the digest by definition. Each regeneration is called out in its PR
  and in the constant's own comment; the determinism contract holds
  otherwise.
