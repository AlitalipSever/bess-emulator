...
Done in PR7, with three revisions the measurement forced. The figures below
are what was measured when those decisions were taken, and they stay as
written; CALIBRATION.md carries the current ones and is regenerated.
...
160 seconds locally, roughly 200 000 ticks per second. Measured on a runner in
its own CI job: 8 minutes 47 seconds including a cold release build, against
6 minutes 34 seconds for the existing checks job running beside it. That is
inside the budget, so the year runs on every pull request rather than on a
sampled window.

One consequence PR8 has to plan for. The record carries the kernel version it
was measured on, so bumping the workspace to 0.3.0 makes `--check` fail on
`run.engine_version` until the year is re-run. That is the intended behavior,
since a release record should name the kernel it describes, but it makes the
release PR a three-minute regeneration rather than a documentation-only
change.
